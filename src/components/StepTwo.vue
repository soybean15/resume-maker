<template>
    ...
    <div class="p-4 my-1 border rounded-md border-current" primary v-for="(item, index) in form.educationalBackground" :key="index + 1">
        ...
        <div class="flex  my-2 flex-wrap gap-2">
            <DatePicker v-model="item.date"  placeholder="From" view="month" dateFormat="mm/yy" />
        ...
        <label for="ingredient1" class="ml-2" > Present </label>
            ...
            <DatePicker v-model="item.date"  :placeholder="item.present=='present' ?'Present':'To'" view="month" dateFormat="mm/yy" :disabled="item.present=='present'" />

        </div>
  
    </div>
    <Button label="Add Background" class="max-w-40"  severity="info" @click="addBackgound" />
